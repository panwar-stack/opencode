...
You can define Docker sandbox profiles for future sandboxed shell execution through the `sandbox` option. Runtime enforcement will be added later; this config currently validates the profiles OpenCode will use for sandboxed shell tool calls.
...
- `network.mode` - `none`, `allowlist`, or `full`. `allowlist` requires a non-empty `hosts` array.
