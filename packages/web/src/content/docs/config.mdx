...
Repository memory lets OpenCode index historical commits, file activity, and cached summaries for the active repository. It is disabled by default.
...
Run [`opencode memory index`](/docs/cli#memory) to create the index. Agent memory tools require both `memory.enabled: true` and an existing index for the active repository.
