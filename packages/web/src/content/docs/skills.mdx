...
OpenCode also ships built-in skills, including `browser-use` for browser automation guidance. Skill access is separate from MCP server access: denying the `browser-use` skill only controls whether the instructions load, while `mcp.browser_use.enabled` and `permission.browser_use_*` control the Browser Use MCP server and tools.
