...
Use `external_directory` to allow tool calls that touch paths outside the working directory where OpenCode was started. This applies to any tool that takes a path as input (for example `read`, `edit`, `glob`, `grep`, `opengrep`, and many `bash` commands).
...
- `opengrep` — semantic code-pattern search (matches the OpenGrep pattern)
- `bash` — running shell commands (matches parsed commands like `git status --porcelain`)
- `task` — launching subagents (matches the subagent type)
- `skill` — loading a skill (matches the skill name)
- `browser_use_*` — Browser Use MCP tools, such as navigation, typing, extraction, uploads, and downloads
- `lsp` — running LSP queries (currently non-granular)
- `question` — asking the user questions during execution
- `webfetch` — fetching a URL (matches the URL)
- `websearch` — web search (matches the query)
- `memory_search_commit` — searching repository commit memory
- `memory_examine_commit` — inspecting a retrieved historical commit memory record
- `memory_search_summary` — searching cached repository file summaries
- `memory_view_summary` — viewing a cached repository file summary
- `external_directory` — triggered when a tool touches paths outside the project working directory
- `doom_loop` — triggered when the same tool call repeats 3 times with identical input

Repository memory permissions only apply after the memory tools are available. Agent memory tools require repository memory to be enabled and an existing index for the active repository. Memory results are historical hints; verify against current source before editing.
...
- `doom_loop`, `external_directory`, and `browser_use_*` default to `"ask"`.
...
To deny and hide Browser Use MCP tools from the model:
