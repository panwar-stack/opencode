...
Repository memory permissions only apply after the memory tools are available. Agent memory tools require `memory.enabled: true` and an existing index for the active repository. Memory results are historical hints; verify against current source before editing.
