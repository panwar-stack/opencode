...
Repository memory methods search historical commit memory and cached file summaries in the v2 SDK. Use results as localization hints and verify against current source before editing.

| Method | Description | Response |
| ------ | ----------- | -------- |
| `memory.index({ memoryIndexRequest? })` | Start a background repository index job | <a href={typesV2Url}><code>MemoryIndexResponse</code></a> |
| `memory.status()` | Get repository memory status | <a href={typesV2Url}><code>MemoryStatusResponse</code></a> |
| `memory.searchCommit({ memorySearchRequest })` | Search historical commit memory | <a href={typesV2Url}><code>MemoryCommitSearchResponse</code></a> |
| `memory.commit({ hash, max_diff_bytes? })` | Examine a historical commit memory record | <a href={typesV2Url}><code>MemoryCommitResponse</code></a> |
| `memory.searchSummary({ memorySearchRequest })` | Search cached file summary memory | <a href={typesV2Url}><code>MemorySummarySearchResponse</code></a> |
| `memory.summary({ path })` | View a cached file summary | <a href={typesV2Url}><code>MemorySummaryResponse</code></a> |
| `memory.clear()` | Clear memory for the active repository | <a href={typesV2Url}><code>MemoryClearResponse</code></a> |
...
await client.memory.index({
  memoryIndexRequest: { max_commits: 7000, summaries: 200 },
})

const status = await client.memory.status()

const commits = await client.memory.searchCommit({
  memorySearchRequest: { query: "authentication regression", limit: 5 },
})
```

Agent-facing memory tools are separate from the SDK. They require repository memory to be enabled and an index for the active repository.
