...
This built-in shell tool allows the LLM to run terminal commands like `npm install`, `git status`, or any other shell command. It can run inside Docker when [`sandbox.enabled`](/docs/config#sandbox) is configured; permissions still decide whether commands may run, while sandboxing controls the runtime environment.
...
Search code using OpenGrep patterns.
...
Semantic code-pattern search powered by OpenGrep. This optional tool is available when the `opengrep` binary can be found or installed.
...
Search repository commit memory for past changes related to one or more natural-language, error-text, code-symbol, or file-path queries.
...
This returns ranked historical commits with hashes, messages, changed files, scores, linked issue titles when available, and whether the match is strong.

:::note
Repository memory tools are available to agents only when repository memory is not disabled and an index exists for the active repository. Memory is historical, so treat results as localization hints and verify against current source before editing.
:::
...
Inspect a retrieved commit memory record.
...
This returns the historical diff, linked issue details when available, changed files, and tests touched. Old line numbers may not match current source.

:::caution
Commit memory is historical. Read current files before patching.
:::
...
Search cached high-activity file summaries by behavior, subsystem, error, function, or file path.
...
This returns ranked file paths with cached summaries, important symbols, scores, and whether the match is strong.
...
This returns the cached summary, important symbols, source hash, generation time, and stale or missing status.
