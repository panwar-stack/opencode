...
:::note
Repository memory tools are available to agents only when `memory.enabled` is `true` and an index exists for the active repository. Memory is historical, so treat results as localization hints and verify against current source before editing.
:::
