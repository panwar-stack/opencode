...
:::note
Repository memory tools require `memory.enabled: true` and an existing index for the active repository. Memory results are historical hints; agents should verify against current source before editing.
:::
