...
OpenCode includes a default local `browser_use` MCP server for browser automation. It runs [BrowserMCP](https://docs.browsermcp.io/) with `npx @browsermcp/mcp@latest`, which lets the BrowserMCP Chrome extension connect an existing Chrome tab to OpenCode.

To use it with an existing Chrome instance, install the BrowserMCP Chrome extension, open Chrome, click the extension icon, and click **Connect**. Browser actions run against the connected tab. Node.js/npm must be available so OpenCode can run `npx`.
...
If the default `@latest` package resolution causes startup problems in your environment, pin the BrowserMCP package command instead:
...
BrowserMCP connection is extension-mediated; there is no `--connect-existing` or CDP flag to add. The server listens locally for the extension, and the extension must be connected before browser tools can control a tab. Ask before using an authenticated existing browser/profile, and do not expose credentials or local profile paths in shared config or logs.
