...
OpenCode includes a default local `browser_use` MCP server for browser automation. It runs through the Python Browser Use package with `uvx` and requests headed browser mode by default.

Homebrew, AUR, Docker, and npm installs include `uv` or package-local `uvx` for this default server. If install scripts are skipped or `uvx` is missing, OpenCode reports a startup error with install instructions.

To disable the default server:
...
Browser Use tools are permissioned with `browser_use_*` and default to `ask`.

Ask before browser actions:
...
Allow browser actions in a trusted project:
...
Deny and hide browser-use MCP tools:
...
If `browser-use` is already installed, you can use the installed command instead of `uvx`:
...
To reuse an existing browser, opt in by adding a CDP URL to the command:
...
OpenCode does not automatically launch Chrome for CDP reuse. Start Chrome yourself before using `--cdp-url`:

```bash
open -a "Google Chrome" --args --remote-debugging-port=9222
```

If you need a reusable profile, prefer a dedicated browser-use profile over your daily personal profile:

```bash
open -a "Google Chrome" --args --remote-debugging-port=9222 --user-data-dir="<path-to-dedicated-browser-use-profile>"
```

Existing browser and profile reuse is always opt-in through explicit `mcp.browser_use.command` config. Do not put credentials, secrets, or real local profile paths in shared config or logs.
