...
export const typesV2Url = `${config.github}/blob/dev/packages/sdk/js/src/v2/gen/types.gen.ts`
...
These endpoints support [repository memory](/docs/memory). Memory results are historical hints; verify commit diffs and summaries against the current working tree before editing.

| Method   | Path                              | Description                               | Response                                                                 |
| -------- | --------------------------------- | ----------------------------------------- | ------------------------------------------------------------------------ |
| `POST`   | `/memory/index`                   | Start a background repository index job   | <a href={typesV2Url}><code>MemoryIndexResponse</code></a>                |
| `GET`    | `/memory/status`                  | Get repository memory status              | <a href={typesV2Url}><code>MemoryStatusResponse</code></a>               |
| `POST`   | `/memory/search/commit`           | Search historical commit memory           | <a href={typesV2Url}><code>MemoryCommitSearchResponse</code></a>         |
| `GET`    | `/memory/commit/:hash`            | Examine a historical commit memory record | <a href={typesV2Url}><code>MemoryCommitResponse</code></a>               |
| `POST`   | `/memory/search/summary`          | Search cached file summary memory         | <a href={typesV2Url}><code>MemorySummarySearchResponse</code></a>        |
| `GET`    | `/memory/summary?path=<file-path>` | View a cached file summary                | <a href={typesV2Url}><code>MemorySummaryResponse</code></a>              |
| `DELETE` | `/memory`                         | Clear memory for the active repository    | <a href={typesV2Url}><code>MemoryClearResponse</code></a>                |
...
These endpoints support [Agent Teams](/docs/agent-teams).

| Method | Path                        | Description                                  | Response                                            |
| ------ | --------------------------- | -------------------------------------------- | --------------------------------------------------- |
| `GET`  | `/team?sessionID=<session>` | Get the active team for a lead session       | <a href={typesV2Url}><code>TeamInfo</code></a>      |
| `GET`  | `/team/{teamID}`            | Get a team by ID                             | <a href={typesV2Url}><code>TeamInfo</code></a>      |
| `GET`  | `/team/{teamID}/eval`       | Get a team evaluation report                 | <a href={typesV2Url}><code>TeamEvalReport</code></a> |
| `GET`  | `/team/{teamID}/tasks`      | List shared team tasks                       | <a href={typesV2Url}><code>TeamTask[]</code></a>    |
| `GET`  | `/team/{teamID}/messages`   | List team messages                           | <a href={typesV2Url}><code>TeamMessage[]</code></a> |
| `POST` | `/team/{teamID}/shutdown`   | Shut down a team and cancel active teammates | `boolean`                                           |
