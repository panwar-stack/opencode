...
While indexing, opencode shows foreground progress for repository resolution, commit crawling, index writes, file activity, and optional file summary generation. The final summary output stays unchanged for scripts.
...
```bash
opencode memory search commit "auth regression" --limit 20
opencode memory search summary "serialization error" --limit 5
```
...
```bash
opencode memory view summary packages/opencode/src/example.ts
```
...
opencode memory clear --repository github.com/owner/repo
```
...
```bash
opencode memory eval --issues issues.json
```
