...
- `memory`: Repository memory can be used during GitHub runs when it is not disabled in `opencode.json` and an index already exists for the repository. OpenCode does not index repository memory on every action run by default. Memory results are historical hints and must be verified against the checked-out source.
