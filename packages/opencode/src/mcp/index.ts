...
function browserUseStartupError(key: string, mcp: ConfigMCP.Info & { type: "local" }, message: string) {
  ...
  return [
    message,
    "The default browser-use MCP server runs BrowserMCP with `npx @browsermcp/mcp@latest`.",
    "Install Node.js/npm if `npx` is unavailable.",
    "To control an existing Chrome tab, install the BrowserMCP Chrome extension, open the extension, and click Connect.",
  ].join("\n")
}
